...
static class Instructions
{
	...
	public static void DrawInstruct ()
	{
		...
		const int WELC_HEADING = 350;
		const int WELC_LEFT = 370;

		if (_Instruct.Count == 0)
			LoadInstruct ();

		SwinGame.DrawText ("   Instructions   ", Color.Black, GameResources.GameFont ("Courier"), INST_LEFT-55, INST_HEADING);

		//Greating
		SwinGame.DrawText ("   WELCOME!   ", Color.DarkBlue, GameResources.GameFont ("Courier"), WELC_LEFT, WELC_HEADING);
		SwinGame.DrawText ("   Dear user, thank you for playing our product!   ", Color.DarkBlue, GameResources.GameFont ("Courier"), WELC_LEFT, WELC_HEADING + INST_GAP);
		SwinGame.DrawText ("   Hope you enjoy BattleShips!   ", Color.DarkBlue, GameResources.GameFont ("Courier"), WELC_LEFT, WELC_HEADING + (INST_GAP * 2));
		...
		for (i = 0; i <= _Instruct.Count - 1; i++) {
			...
			if (i < 8) {
				SwinGame.DrawText (s.Name, Color.Black, GameResources.GameFont ("Courier"), INST_LEFT-55, INST_TOP + i * INST_GAP);
			} else {
				SwinGame.DrawText (s.Name, Color.Black, GameResources.GameFont ("Courier"), INST_LEFT-55, INST_TOP + i * INST_GAP);
			}
		}
